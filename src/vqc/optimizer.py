...
#SPSA - it wil nudge every parameter randomly, check if it helped, then step in whichever direction is better

delta = np.random.choice([-1,1], size = len(theta)) 
#a coin flip for every parameter (+1 means try nudging it up and -1 meeans try nudging it down)
...
#compares the two errors to guess which way is downhill for each parameter at once
