...
# same gates as IBM machine
IBM_GATES = ['rz', 'sx', 'x', 'cx']


def show(name, qc): # prints one row of the table so we dont repeat this 5 times
    # transpile swaps our gates for ones real hardware actually has (also btw hw is hardware)
...
def main():
    ...
    x = np.array([0.7, 1.3])
    z = np.array([2.1, 0.4])
    print("note: hw is hardware\n-------\n")
    print(f"{'circuit':<27} {'qubits':<7} {'depth':<7} {'gates':<7} {'hw depth':<9} hw gates")

    show("feature map", feature_map_circuit(x))
    show("overlap (1 kernel entry)", circuit_overlap(x, z)) # feature map forward then backward
    ...
    print("\n amt of circuits needed per run -")

    # kernel only needs the top half of the matrix bc both K(x,z) and K(z,x) are the same
    for points in [10, 20, 30, 40]:
        pairs = points * (points - 1) // 2
        print(f"  kernel, {points} training pts: {pairs} circuits ({pairs * 4096:,} shots total)")

    # the vqc runs one circuit per point but it has to redo all of them every optimizer step
    for iters in [50, 200]:
        circuits = 30 * iters
        print(f"  vqc, both are just 30 training points, {iters} iters: {circuits} circuits ({circuits * 1024:,} shots total)")
